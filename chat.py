...
class Chat(commands.Cog):
    ...
    async def on_guild_join(self,guild):
        async for member in await guild.fetch_members():
    ...
    async def on_member_update(self,before,after):
        if discord.activity.Spotify in list(map(type,after.activities)):
          ...
          if self.users.find_one({"_id": after.id})["bef_track_id"] != af_activ.track_id or self.users.find_one({"_id": after.id})["bef_track_id"] is None:
            ...
            for guild in guilds:
              channel_id = int(guilds_channels[str(guild.id)])
              if channel_id is not None:
                channel = discord.utils.get(after.guild.channels,id = channel_id)
                await channel.send(file = discord.File(fp = _buffer,filename = f'{after.name}spotify.png'))
    ...
    async def toptracks(self,ctx):
        embed = discord.Embed(title = f'The most popular tracks in {ctx.guild.name}',color = discord.Color.green())
        songs = self.collection.find({"guild_id" : ctx.guild.id},limit = 10).sort("song_auditions",-1)
        i = 0
        print(songs)
        for song in songs:
          i += 1
          embed.add_field(name = f'{", ".join(songs["authors"])} - {song["title"]}',value = f'Auditions: **{song["song_auditions"]}**',inline = False)
